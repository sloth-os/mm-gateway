...
- **Music (Lyria 3)**: REST `POST {music_base}/v1beta/models/{model}:predictInteractions?key={api_key}` over a per-request `httpx.AsyncClient(timeout=240.0)` — the SDK does not yet expose Lyria ergonomically. Auth = api_key as a `?key=` query param. `music_base` = `backend.extra["music_base_url"]` or `backend.base_url` or `https://generativelanguage.googleapis.com`.
  - Models: `lyria-3`.
  - Body: `{model, input: parts[], config}`. Canonical text, image, and audio parts become native input parts. Data URLs are decoded into inline `{type, mime_type, data}` blocks; ordinary references remain URL blocks. `config` contains `response_modalities:["AUDIO"]` plus the translated file format, negative prompt, seed, guidance, and output count.
...
- **Music**: Genuine two-phase async REST plus a binary download.
  ...
  - Body (`_build_body`): `prompt`, structured `lyrics`, vocal language, translated file format, duration, bpm, combined key and scale, time signature, guidance, seed, and model. Neutral inference steps and output count map to `inference_steps` and `batch_size`; a continuation or reference audio part maps to `reference_audio_path`.
